function [full_visualization, full_key, full_matrix_no, full_anno_lst] ...
    = hierarchical_structure_with_vis(matrix_no, key_no, sn)
...
% hierarchical representation. We also produce images for several steps of
% the process. 
%
...
% This gives a visualization for PNO
[PNO_anno] = get_annotation_lst(PNO_key);
[PNO_ylabels] = get_yLabels(PNO_key, PNO_anno);
num_PNO_rows = size(PNO,1);
figure(); imagesc(2-(PNO_block+PNO)); colormap(gray);
    set(gca,'YTick', [1:num_PNO_rows]'); set(gca,'YTickLabel',PNO_ylabels);
...
% Assign ONE new unique color to all the zero block 
...
figure(); imagesc(symm_PNO_inds_only); colormap(vga)
...
figure(); imagesc(NZI_pattern_block); colormap(vga)
figure(); imagesc(NZI_matrix_no + NZI_pattern_block); colormap(vga)
...
% This gives a visualization for FULL_VISUALIZATION with the starts of the
% blocks marked by FULL_MATRIX_NO
[vis_ylabels] = get_yLabels(full_key, full_anno_lst);
figure(); imagesc(2-(full_visualization+full_matrix_no)); colormap(gray);
    set(gca,'YTick', [1:nzi_rows]'); set(gca,'YTickLabel',vis_ylabels);