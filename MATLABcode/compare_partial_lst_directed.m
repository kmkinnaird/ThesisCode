function partial_lst_distmat = ...
    compare_partial_lst_directed(compare_songlist)

% COMPARE_PARTIAL_LIST_DIRECTED computes the matrix of pairwise comparisons
% between the representations (or lists of representations) listed in
...
% the total number of representations in file I.
%
    ...
    if (I_num_reps > 0)
        for j = (i + 1): num_songs
            ...
            if (J_num_reps > 0)
                ...
                % So we leave D(i,j) = 0 and D(j,i) = 0.
                if ~isempty(I_inds)
                    ...
                    num_of_exact_comps = size(I_exact,1);
                    
                    D(i,j) = num_of_exact_comps/I_num_reps;
                    
                    D(j,i) = num_of_exact_comps/J_num_reps;
                end
            end
        end
    end
